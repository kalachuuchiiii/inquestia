...
import { useSurveyAnswers } from "../../answers/hooks/useSurveyAnswers";
import { AnswerFormCard } from "../../answers/components/AnswerFormCard";
import { SurveyAnswerCard } from "../../answers/components/SurveyAnswerCard";
import api from "@/lib/axios.instance";

const SurveyAnalyticsPage = () => {
  const { survey, answers, filterForm, filterFormControl, applyFilters } =
    useSurveyAnswers();
  ...
  // Fetch statistics based on isAuthentic filter
        ...
        <Card className="bg-gradient-to-r from-blue-50 via-cyan-50 to-blue-100 dark:from-zinc-800 dark:via-zinc-900 dark:to-zinc-800 border-blue-200 dark:border-zinc-700">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-zinc-600 dark:text-zinc-400">
              View statistics for all responses, only authentic, or only
              non-authentic answers
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="space-y-3">
          {answers.map((ans, idx) => (
            ...
            >
              <SurveyAnswerCard
                answer={ans}
                user={ans?.respondent as UserDTO}
              />
            </motion.div>
          ))}
        </div>
