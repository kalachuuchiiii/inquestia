...
export class AuthController {
  ...
  getAccount: RequestHandler = async (req, res) => {
    const myId = ObjectIdSchema.parse(req.myId);

    const { user } = await authService.getUserData({ myId });

    const cleanUser = UserWithCredentialSchema.parse(user);
    const response = {
      user: cleanUser,
      success: true,
    };
  ...
  register: RequestHandler = async (req, res) => {
    const { email, password, username, code } = RegisterFormSchema.extend({
      code: CodeSchema,
    })
      .strip()
      .parse(req.body);
  ...
  login: RequestHandler = async (req, res) => {
    ...
    const token = jwt.sign({ myId: user._id }, ENV_CONFIG.JWT_SECRET, {
  ...
  updatePassword: RequestHandler = async (req, res) => {
    const myId = ObjectIdSchema.parse(req.myId);
    const { password: newPassword } = LoginFormSchema.pick({
      password: true,
    }).parse(req.body);
