...
const AnswerCard = ({
  ...
  const getAns = (question) => {
    ...
    if (typeof ans === "string") {
      return (
        <div className="p-2 rounded-lg w-full  dark:bg-gray-800 bg-blue-200 text-zinc-900 dark:text-neutral-100">
          {ans ? (
            <p>{ans}</p>
          ) : (
            <p className="opacity-50">No answer provided</p>
          )}
        </div>
      );
    }
  ...
  return (
    <div className="p-6 w-full shadow-xl dark:bg-zinc-900 rounded-xl outline outline-neutral-200/20 dark:outline-neutral-800/40 m-2">
      {/* User */}
      ...
      {/* Answers */}
      <div className="rounded-b-lg space-y-4 p-3 mt-2">
        ...
        <div className="flex  justify-start">
          ...
          {showRedirectToAnswer && (
            <div className="w-full flex justify-end pt-2">
              <ArrowButton to={`/answer/${answer?._id}`}>
                View Answer
              </ArrowButton>
            </div>
          )}
