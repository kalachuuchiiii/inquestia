...
const AnswerCard = ({ showRedirect = false, answer = {
  ...
  const getAns = (question) => {
    ...
    if (typeof ans === "string") {
      return <p className="p-2 bg-zinc-900 rounded-lg w-full">{ans}</p>
    }
      ...
      <div className="w-full space-y-2">
        <div className="space-y-1">
          {question.choices.map((c) => (
            <p
              className={`${
                normalizedAns.includes(c.trim().toLowerCase()) &&
                " bg-neutral-50 text-zinc-900 w-full"
              } px-3 py-1 rounded `}
            >
              {c}
            </p>
  ...
  return (
    <div className="p-1 bg-zinc-900 rounded-xl outline-1 outline-neutral-100/50 m-2">
      ...
      <div className="rounded-b-lg  space-y-2 bg-zinc-950 p-2">
        {answer?.survey?.questions?.length > 0 &&
          answer.survey.questions.map((q, i) => (
            <div key={q._id} className="text-sm space-y-3 ">
              <div>
                <p className="lato text-base">
                  {i + 1}.) {q.question}
                </p>
                <p>{capitalize(questionType[q.type])}</p>
                {q?.multipleChoice && (
                  <p className="text-xs px-3 py-1  w-fit">Multiple Choice</p>
                )}
              </div>
