...
const QueryUsers = () => {
  ...
  }, [nextPage, isSearchInactive, isLoading, inView, ref])

  return <div className = "space-y-1"  >
    {
     (!isLoading && isNoResultsFound) ? <div className = "flex opacity-70 justify-center items-center h-96 ">
       No results found
     </div> : users?.length > 0 && users.map((user) => <div className="bg-zinc-900 rounded-lg p-2  w-full flex flex-col gap-2" key={user._id}>
        <User className="text-sm p-2" user={user}>
          <User.Card size="8" />
        </User>
        <NavLink className="w-full p-2 bg-zinc-800 text-sm rounded-lg text-neutral-100" to = {sessionUser._id === user._id ? "/profile" : `/users/${user.username}`}>
          View Profile
        </NavLink>
      </div>)
    }
